...
if [[ -s "${PROJECT_DIR}"/.github_token ]]; then
	...
	if [[ "${GIT_ORG}" == "${GIT_USER}" ]]; then
		...
		curl -s -X POST -H "Authorization: token ${GITHUB_TOKEN}" -d '{"name": "'"${repo}"'", "private":true, "description": "'"${description}"'"}' "https://api.github.com/orgs/${GIT_ORG}/repos" >/dev/null 2>&1
	fi
